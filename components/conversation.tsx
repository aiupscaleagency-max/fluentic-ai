        ...
        >
          {messages.length === 0 && (
            <div className="text-center text-slate-500 py-12">
              {persona
                ...
                : <>Säg hej till Hectór på {language.name.toLowerCase()}!</>}
              <div className="mt-2 text-xs">
                Hectór svarar på {language.name.toLowerCase()} med översättning under.
              </div>
            </div>
          )}
          {messages.map((m, i) => (
          ...
          {loading && (
            <div className="flex items-center gap-2 text-sm text-slate-500">
              <Loader2 className="h-4 w-4 animate-spin" /> Hectór tänker…
            </div>
          )}
          {error && (
